            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="skuNum"
                  @click="(event) => changeSkuNum(event)"
                />
              ...
              <div class="add">
                <!-- 
                  一般的路由跳转从A路由跳到B路由 这次需要在加入购物车 进行路由跳转前 
                  发请求 将产品信息通过请求通知服务器 服务器进行相应存储
                 -->
...
// 产品的售卖属性值切换高亮
function changeActive(saleAttrValue, arr) {
...
async function addShopcar() {
  // 派发一个action 向服务器发请求 判断加入购物车成功或者失败 进行相应操作
  // 调用仓库中的addOrUpdateShopCart 这个方法加上async 返回一定是一个Promise
  try {
    store.dispatch("addOrUpdateShopCart", {
      skuId: route.params.skuid,
      skuNum: skuNum.value,
    });
    // 路由跳转的时候 将产品信息传递给下个路由
...
const stores = computed(()=> store.state.detail)
