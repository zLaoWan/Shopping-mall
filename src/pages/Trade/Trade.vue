...
function changeDefault(address, addressInfo) {
  ...
  address.isDefault = 1;
  console.log(userDefaultAddress, 111)
}
async function sumbitOrder() {
  // 交易编码
  let { tradeNo } = orderInfo.value
  // 六个参数
  let data = {
    consignee: userDefaultAddress.value.consignee,  // 最终收件人的名字
    consigneeTel: userDefaultAddress.value.phoneNum,  // 最终收件人的手机号
    deliveryAddress: userDefaultAddress.value.fullAddress,  // 收件人地址
    paymentWay: "ONLINE", // 支付方式
    orderComment: msg.value,  // 买家留言信息
...
// 提交订单最终选中地址   find：查找数组当中符合条件的元素返回 作为最终结果
const userDefaultAddress = computed(() => addressInfo.value.find((item) => item.isDefault == 1) || {})
</script>
